...
async def run_command_async(cmd: list[str], cwd: str = "/home/jpeetz",
    ...
    """Async wrapper around run_command that offloads to a thread so the
    event loop is not blocked by long-running subprocesses."""
    import functools
    fn = functools.partial(
        _run_sync_with_stdin if stdin_data is not None else run_command,
...
def _run_sync_with_stdin(cmd: list[str], cwd: str = "/home/jpeetz",
                          timeout: int = 300, env: dict | None = None,
                          stdin_data: bytes | None = None) -> dict:
    """Synchronous subprocess runner with stdin piping.
    Used by run_command_async for tools that need stdin input."""
    start = time.monotonic()
    merged_env = os.environ.copy()
    if env:
        merged_env.update(env)
